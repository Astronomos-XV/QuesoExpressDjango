...
class SuperuserForm(forms.ModelForm):
    superuser_code = forms.CharField(
        required=False,
        widget=forms.PasswordInput(attrs={'placeholder': 'Solo para crear superusuarios'}),
    ...
    # ------------------------------------------------------------------
    # 1. Validación del código maestro (el más importante)
    # ------------------------------------------------------------------
    def clean_superuser_code(self):
        code = self.cleaned_data.get('superuser_code', '').strip()
        if not code:
            return None  # Es usuario normal

        if not CodigoActivacionSuperuser.validar_codigo(code):
            raise forms.ValidationError("Código de activación inválido o incorrecto.")
        return code

    # ------------------------------------------------------------------
    # 2. Lógica: superusuario vs usuario normal
    # ------------------------------------------------------------------
    def clean(self):
    ...
    def save(self, commit=True):
        ...
        if es_superusuario:
            ...
            user.is_staff = True  # permite entrar al admin (opcional)
