...
from .models import Empleado,  Tasa, Nomina, Sueldo, TipoDeJornada, Departamento, Profesion, Labor, TipoDeJornada, Asistencia, Asistenciaconfirmada, Usuarios, PrestacionSocialAcumulada,  Liquidacion, Prestamo,ConceptoNomina,ParametrosNomina, SueldoLabor, CambioLabor, BonoExtra, ParametrosNomina, DetalleConceptoNomina
from .forms import EmpleadoForm, SueldoForm, SuperuserForm, DepartamentoForm, ProfesionForm, LaborForm, TipoDeJornadaForm, NominaForm, BuscarNominaForm, AsistenciaForm, NominaGrupalForm, SueldoGrupalForm, LiquidacionGrupalForm, LiquidacionForm,PrestamoForm, ParametrosNominaForm, SueldoLaborForm
...
def home(request):
    ...
    """
    nominas_list = Nomina.objects.all().order_by('-fecha_creacion', 'id_nomina') 
...
def generar_nomina_grupal(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            DIAS_PERIODO = 7
            fecha_fin = fecha_inicio + timedelta(days=DIAS_PERIODO - 1)
            dias_en_periodo = DIAS_PERIODO
            DIAS_MES_PROMEDIO = Decimal('30.0') 
            
            # Factores de pago asumidos (50% y 100% de recargo sobre la hora ordinaria)
           # FACTOR_HORA_EXTRA = Decimal('1.50') 
            #FACTOR_HORA_FESTIVA = Decimal('2.00') 
            
            try:
                ultima_tasa = Tasa.objects.latest('fecha') 
                valor_tasa_por_falta = ultima_tasa.valor_tasa
            except Tasa.DoesNotExist:
                messages.error(request, "No se ha definido ninguna Tasa de cambio.")
                return render(request, 'generar_nomina_grupal.html', {'form': form})
            
          
            try:
                # --- Parámetros Obligatorios: (IVSS, RPE, FAOV, INCES, CESTATICKET, UT)
                parametros_fijos = ParametrosNomina.objects.filter(es_concepto_fijo=True)
                
                IVSS_PARAM = parametros_fijos.get(nombre__icontains='ivss')
                RPE_PARAM = parametros_fijos.get(nombre__icontains='rpe')
                FAOV_PARAM = parametros_fijos.get(nombre__icontains='faov')
               # INCES_PARAM = parametros_fijos.get(nombre__icontains='ince') 
                CESTATICKET_PARAM = parametros_fijos.get(nombre__icontains='cestaticket')
 
                FACTOR_EXTRA_PARAM = parametros_fijos.get(nombre__icontains='hora extra')
                FACTOR_FESTIVA_PARAM = parametros_fijos.get(nombre__icontains='hora festiva')
                FACTOR_HORA_EXTRA = FACTOR_EXTRA_PARAM.porcentaje
                FACTOR_HORA_FESTIVA = FACTOR_FESTIVA_PARAM.porcentaje
                if CESTATICKET_PARAM.monto_fijo_usd and CESTATICKET_PARAM.monto_fijo_usd > 0:
                    cestaticket_mensual_bs = CESTATICKET_PARAM.valor_en_bs()  # ← método del modelo (usa última tasa)
                else:
                    messages.error(request, "Error: El cestaticket debe tener un monto en USD configurado (monto_fijo_usd).")
                    return render(request, 'generar_nomina_grupal.html', {'form': form})
            except ParametrosNomina.DoesNotExist as e:
                messages.error(request, f"Faltan parámetros de cálculo esenciales. Error: {e}")
                return render(request, 'generar_nomina_grupal.html', {'form': form})
            ...
            first_day_of_fecha_fin_month = date(fecha_fin.year, fecha_fin.month, 1)

            if fecha_fin.day <= RANGO_CIERRE:
                last_day_of_month = first_day_of_fecha_fin_month - timedelta(days=1)
                first_day_next_month = first_day_of_fecha_fin_month
            else:
                last_day_of_month = date(fecha_fin.year, fecha_fin.month, calendar.monthrange(fecha_fin.year, fecha_fin.month)[1])
                if fecha_fin.month == 12:
                    first_day_next_month = date(fecha_fin.year + 1, 1, 1)
                else:
                    first_day_next_month = date(fecha_fin.year, fecha_fin.month + 1, 1)

            start_eom_window = last_day_of_month - timedelta(days=RANGO_CIERRE - 1)
            is_in_eom_window = start_eom_window <= fecha_fin <= last_day_of_month
            end_som_window = first_day_next_month + timedelta(days=RANGO_CIERRE - 1)
            is_in_som_window = first_day_next_month <= fecha_fin <= end_som_window
            ultimo_dia_mes = is_in_eom_window or is_in_som_window
            mes_referencia = last_day_of_month.month 
            es_fin_de_trimestre = mes_referencia in [3, 6, 9, 12] and ultimo_dia_mes
            es_fin_de_anio = mes_referencia == 12 and ultimo_dia_mes
            
            #INCES_PARAM,
            reglas_a_aplicar_fijas = [IVSS_PARAM, RPE_PARAM, FAOV_PARAM,  CESTATICKET_PARAM]
            reglas_personalizadas = ParametrosNomina.objects.filter(es_concepto_fijo=False)
            
            reglas_a_aplicar = list(reglas_personalizadas)
            reglas_a_aplicar.extend(reglas_a_aplicar_fijas)
            ...
            for empleado in empleados_por_labor:
                ...
                total_asignaciones_custom = Decimal('0.00')
                total_deducciones_custom = Decimal('0.00')
                total_asignaciones_obligatorias = Decimal('0.00')
                total_deducciones_obligatorias = Decimal('0.00')

                cestaticket_bs_calculado_empleado = Decimal('0.00')
                ivss_bs = Decimal('0.00')
                rpe_bs = Decimal('0.00')
                faov_bs = Decimal('0.00')
                inces_bs = Decimal('0.00')
                
                conceptos_personalizados_aplicados = []

                try:
                    sueldo_empleado = Sueldo.objects.filter(id_empleado=empleado).latest('fecha_creacion') 
                except Sueldo.DoesNotExist:
                    errores_generacion.append(f"Error: Empleado {empleado.nombre} {empleado.apellido} no tiene sueldo registrado.")
                    continue

                # Sueldo semanal en Bs que corresponde al período
                sueldo_bs_base_periodo = sueldo_empleado.sueldo_bs 
                
                # Inicialización de variables de pago/deducción
                total_pago_horas_extras_bs = Decimal('0.00')
                total_pago_horas_festivas_bs = Decimal('0.00')
                total_pago_bonos_bs = Decimal('0.00')
                pago_inasistencias_bs = Decimal('0.00') 
                total_deduccion_prestamo_bs = Decimal('0.00')
                total_faltas_empleado = 0 
                
                # Variables para registro en ConceptoNomina
                total_horas_ordinarias_confirmadas = Decimal('0.00') 
                total_horas_extras = Decimal('0.00') 
                total_horas_festivas = Decimal('0.00') 
                
                # -------------------------------------------------------------
                # 🌟 CÁLCULOS DE TARIFA BASE, DEDUCCIONES Y ASIGNACIONES POR ASISTENCIA
                # -------------------------------------------------------------
                
                # 1. Obtener la Jornada Ordinaria REAL del modelo TipoDeJornada
                try:
                    # Este valor debe ser el DecimalField 'horas_diarias' del modelo TipoDeJornada
                    JORNADA_ORDINARIA_HORAS = empleado.id_jornada.horas_diarias 
                except TipoDeJornada.DoesNotExist:
                    messages.error(request, f"Error: Empleado {empleado.nombre} no tiene una jornada laboral asignada.")
                    continue
                
                # 2. Calcular el sueldo diario y por hora (basado en 30 días y la jornada real)
                try:
                    # Cálculo diario basado en un mes de 30 días
                    sueldo_diario_bs = sueldo_bs_base_periodo / DIAS_MES_PROMEDIO
                    # Valor hora ordinaria usa la jornada real del empleado
                    valor_hora_ordinaria_bs = sueldo_diario_bs / JORNADA_ORDINARIA_HORAS 
                except InvalidOperation:
                    sueldo_diario_bs = Decimal('0.00')
                    valor_hora_ordinaria_bs = Decimal('0.00')
                
                # --- A. Lógica para el Cálculo de Faltas (DEDUCCIÓN) - Tabla Asistencia
                
                # Obtener el total de inasistencias injustificadas en el período
                inasistencias_periodo = Asistencia.objects.filter(
                    id_empleado=empleado,
                    fecha_asistencia__range=[fecha_inicio, fecha_fin],
                    tipo_inasistencia='injustificada'
                ).count()

                total_faltas_empleado = inasistencias_periodo

                # Calcular la deducción total (sueldo diario * número de faltas)
                pago_inasistencias_bs = sueldo_diario_bs * Decimal(total_faltas_empleado)

                # --- B. Lógica para el Cálculo de Horas Extras/Festivas (ASIGNACIÓN) - Tabla Asistenciaconfirmada (SOLUCIÓN AL FIELD ERROR)
                
                # Usamos filter para obtener los registros y LUEGO iteramos en Python
                registros_confirmados = Asistenciaconfirmada.objects.filter(
                    id_empleado=empleado,
                    fecha_asistencia__range=[fecha_inicio, fecha_fin] # <--- Campo de fecha correcto
                ).order_by('fecha_asistencia') 
                
                
                for registro in registros_confirmados:
                    
                    # Usamos los campos correctos: hora_entrada, hora_salida
                    if registro.hora_entrada and registro.hora_salida:
                        
                        # 🌟 SOLUCIÓN AL ERROR: Combina DateField y TimeField para permitir la resta
                        dt_entrada = dt_mod.datetime.combine(registro.fecha_asistencia, registro.hora_entrada)
                        dt_salida = dt_mod.datetime.combine(registro.fecha_asistencia, registro.hora_salida)

                        print(dt_entrada)
                        print(dt_salida)

                        # Manejo de turnos nocturnos (salida al día siguiente)
                        if dt_salida < dt_entrada:
                            dt_salida += timedelta(days=1)
                            
                        tiempo_trabajado = dt_salida - dt_entrada
                        
                        # Convertir timedelta a horas como Decimal
                        horas_trabajadas_decimal = Decimal(tiempo_trabajado.total_seconds() / 3600).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP)
                        print(horas_trabajadas_decimal)
                        
                        horas_extras_dia = Decimal('0.00')
                        horas_festivas_dia = Decimal('0.00')
                        horas_ordinarias_dia = Decimal('0.00')

                        # 1. Determinar si es día festivo/fin de semana (Sáb=5, Dom=6)
                        es_dia_festivo = registro.fecha_asistencia.weekday() >= 5 

                        if es_dia_festivo:
                            # Si es festivo/fin de semana, todas las horas trabajadas son festivas
                            horas_festivas_dia = horas_trabajadas_decimal
                        else:
                            if horas_trabajadas_decimal > JORNADA_ORDINARIA_HORAS:
                                horas_extras_dia = horas_trabajadas_decimal - JORNADA_ORDINARIA_HORAS
                                horas_ordinarias_dia = JORNADA_ORDINARIA_HORAS
                            else:
                                horas_ordinarias_dia = horas_trabajadas_decimal

                        total_horas_ordinarias_confirmadas += horas_ordinarias_dia
                        total_horas_extras += horas_extras_dia
                        total_horas_festivas += horas_festivas_dia
                
                
                pago_extras_calculado = total_horas_extras * valor_hora_ordinaria_bs * FACTOR_HORA_EXTRA
                pago_festivas_calculado = total_horas_festivas * valor_hora_ordinaria_bs * FACTOR_HORA_FESTIVA
                
                total_pago_horas_extras_bs = pago_extras_calculado
                total_pago_horas_festivas_bs = pago_festivas_calculado


                # --- C. Lógica para el Pago de Bonos Extras (ASIGNACIÓN) - Tabla BonoExtra
                bonos_pendientes = BonoExtra.objects.filter(
                    id_empleado=empleado,
                    fecha_aplicacion__range=[fecha_inicio, fecha_fin],
                    pagado_en_nomina=False
                )
                total_pago_bonos_bs = bonos_pendientes.aggregate(Sum('monto_bs'))['monto_bs__sum'] or Decimal('0.00')

                # Marcar bonos como pagados
                if total_pago_bonos_bs > Decimal('0.00'):
                    bonos_pendientes.update(pagado_en_nomina=True)


                # --- D. Lógica para la Deducción de Préstamos (DEDUCCIÓN) - Tabla Prestamo
                try:
                    prestamo_activo = Prestamo.objects.get(id_empleado=empleado, activo=True)
                    monto_cuota = prestamo_activo.monto_cuota_bs

                    if prestamo_activo.monto_pendiente_bs >= monto_cuota and prestamo_activo.cuotas_restantes > 0:
                        total_deduccion_prestamo_bs = monto_cuota
                        
                        # Aplicar la deducción
                        prestamo_activo.monto_pendiente_bs -= monto_cuota
                        prestamo_activo.cuotas_restantes -= 1
                        
                        # Verificar si el préstamo se ha liquidado
                        if prestamo_activo.cuotas_restantes <= 0 or prestamo_activo.monto_pendiente_bs <= Decimal('0.00'):
                    ...
                    messages.warning(request, f"Advertencia: Múltiples préstamos activos para {empleado.nombre}. Se omite la deducción de préstamo.")
                    pass
                
                # -------------------------------------------------------------
                
                base_calculo_porcentajes = sueldo_bs_base_periodo
                base_calculo_social = sueldo_bs_base_periodo 
                ...
                for regla in reglas_a_aplicar: 
                    ...
                    periodicidad = regla.periodicidad.upper() 
                    
                    if periodicidad == TIPO_PERIODO: 
                        aplicar_regla = True
                    elif periodicidad == 'QUINCENAL' and (ultimo_dia_mes or fecha_fin.day in [14, 15, 16]): 
                        aplicar_regla = True 
                    elif periodicidad == 'MENSUAL' and ultimo_dia_mes:
                        aplicar_regla = True
                    elif periodicidad == 'TRIMESTRAL' and es_fin_de_trimestre:
                        aplicar_regla = True
                    elif periodicidad == 'ANUAL' and es_fin_de_anio:
                        aplicar_regla = True

                    if aplicar_regla:
                        
                        if regla.es_concepto_fijo:
                            
                            if 'cestaticket' in regla.nombre.lower():
                                # Calcular la porción semanal del cestaticket mensual
                                monto_aplicado = cestaticket_mensual_bs
                                cestaticket_bs_calculado_empleado = monto_aplicado
                                total_asignaciones_obligatorias += monto_aplicado
                                
                            elif regla.tipo == 'DEDUCCION':
                                if regla.porcentaje and regla.porcentaje > Decimal('0.00'):
                                    # Se asume que las deducciones sociales se calculan sobre la base (sueldo_bs_base_periodo)
                                    monto_aplicado = base_calculo_social * regla.porcentaje
                                    
                                    if 'ivss' in regla.nombre.lower():
                                        ivss_bs = monto_aplicado
                                    elif 'rpe' in regla.nombre.lower():
                                        rpe_bs = monto_aplicado
                                    elif 'faov' in regla.nombre.lower():
                                        faov_bs = monto_aplicado
                                    #elif 'ince' in regla.nombre.lower():
                                        #inces_bs = monto_aplicado 
                                        
                                    total_deducciones_obligatorias += monto_aplicado
                                
                        else: 
                            if regla.monto_fijo and regla.monto_fijo > Decimal('0.00'):
                                monto_aplicado = regla.monto_fijo
                            
                            elif regla.porcentaje and regla.porcentaje > Decimal('0.0000'):
                                monto_aplicado = base_calculo_porcentajes * regla.porcentaje

                            if monto_aplicado > Decimal('0.00'):
                                if regla.tipo == 'ASIGNACION':
                                    total_asignaciones_custom += monto_aplicado
                                elif regla.tipo == 'DEDUCCION':
                                    total_deducciones_custom += monto_aplicado
                                
                                conceptos_personalizados_aplicados.append({
                                    'regla': regla,
                ...
                total_asignaciones_bs = total_asignaciones_base + total_asignaciones_obligatorias + total_asignaciones_custom
                
                total_deducciones_base = (
                    pago_inasistencias_bs + 
                    total_deduccion_prestamo_bs
                )
                
                total_deducciones_bs = total_deducciones_base + total_deducciones_obligatorias + total_deducciones_custom
                ...
                # CREACIÓN DE CONCEPTOS FIJOS/BASE
                ConceptoNomina.objects.create(
                    id_nomina=nomina,
                    sueldo_bs_base=round(sueldo_bs_base_periodo, 2),
                    cestaticket_bs=round(cestaticket_bs_calculado_empleado, 2), 
                    ...
                    pago_bonos_bs=round(total_pago_bonos_bs, 2),
                    total_asignaciones_bs=round(total_asignaciones_bs, 2), 
                    total_deducciones_bs=round(total_deducciones_bs, 2), 
                )
                
                # CREACIÓN DE CONCEPTOS PERSONALIZADOS
                for concepto in conceptos_personalizados_aplicados:
                    if concepto['monto'] > Decimal('0.00'):
                        DetalleConceptoNomina.objects.create(
                            id_nomina=nomina,
                            id_regla=concepto['regla'],
                            nombre_concepto=concepto['regla'].nombre,
                            tipo_concepto=concepto['regla'].tipo,
                            monto_calculado_bs=round(concepto['monto'], 2)
                        )
                        
                nominas_generadas.append(nomina)
                
            messages.success(request, f"{len(nominas_generadas)} nóminas generadas con éxito.")
            ...
            messages.error(request, "Error en los datos de entrada para la generación de nómina.")
            return render(request, 'generar_nomina_grupal.html', {'form': form})
...
def generar_nomina_grupal(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            labor_seleccionada = form.cleaned_data['labor']
            fecha_inicio = form.cleaned_data['fecha_inicio']
            fecha_fin = fecha_inicio + timedelta(days=6)  # Siempre semanal

            # =================================================================
            # 1. CARGA DE PARÁMETROS Y TASA BCV
            # =================================================================
            try:
                ultima_tasa = Tasa.objects.latest('fecha')
            except Tasa.DoesNotExist:
            ...
            required = ['ivss', 'rpe', 'faov', 'cestaticket socialista', 'hora extra', 'hora festiva']
            for req in required:
            ...
            # =================================================================
            # 2. ¿ES FIN DE MES? → Para cestaticket y prestaciones
            # =================================================================
            ...
            for empleado in empleados:
                ...
                for reg in confirmadas:
                    if reg.hora_entrada and reg.hora_salida:
                        ...
                        if reg.fecha_asistencia.weekday() >= 5:  # sábado/domingo
                ...
                # Si no hay confirmadas → usar días asistidos simples
                ...
                # -----------------------------------------------------------------
                # DEDUCCIONES LEGALES (sobre salario devengado real)
                # -----------------------------------------------------------------
                ...
                # -----------------------------------------------------------------
                # ACUMULAR PRESTACIONES (solo fin de mes)
                # -----------------------------------------------------------------
                if es_fin_de_mes:
                    alicuota_utilidades = Decimal('0.25')    # 90 días / 360
                    alicuota_bono_vacacional = Decimal('0.0417')  # 15 días / 360
